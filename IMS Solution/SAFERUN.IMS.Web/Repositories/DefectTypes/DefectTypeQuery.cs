...
{
   public class DefectTypeQuery:QueryObject<DefectType>
    {
        public DefectTypeQuery WithAnySearch(string search)
        {
		...
		public DefectTypeQuery WithPopupSearch(string search,string para="")
        {
            if (!string.IsNullOrEmpty(search))
                And( x =>  x.Name.Contains(search) || x.Code.Contains(search) || x.Description.Contains(search) );
            return this;
		...
		public DefectTypeQuery Withfilter(IEnumerable<filterRule> filters)
        {
           if (filters != null)
           {
