...
{
   public class WorkTypeQuery:QueryObject<WorkType>
    {
        public WorkTypeQuery WithAnySearch(string search)
        {
		...
		public WorkTypeQuery WithPopupSearch(string search,string para="")
        {
            if (!string.IsNullOrEmpty(search))
                And( x =>  x.Name.Contains(search) || x.Description.Contains(search) );
            return this;
		...
		public WorkTypeQuery Withfilter(IEnumerable<filterRule> filters)
        {
           if (filters != null)
           {
